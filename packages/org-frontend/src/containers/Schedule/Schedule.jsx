      ...
      postings.map((posting) => {
        ...
        posting.timeslots.map((time) => {
          if (
            ...
            isFound = true;
            pendingPostings.push({ ...posting, complete: false });
          }
        });
      });
      completePostings.map((posting) => {
        ...
        posting.timeslots.map((time) => {
          if (
            time.applicants.some(
              (applicant) =>
                applicant.status === "selected" || applicant.status === "signed"
            ) &&
            !isFound
          ) {
            isFound = true;
            pendingPostings.push({ ...posting, complete: true });
          }
        });
      });
          ...
          tileContent={({ date, view }) => {
            ...
            if (dates.length > 0) {
              ...
              return dates.map((x) => (
                <div
                  key={x.title}
                  className={`overflow-hidden text-[8px] ${
            ...
            timeslot.map((time) =>
              time.timeslots.map((x, index) => {
                ...
                return (
                  <div
                    key={time.id + "-" + index}
                    className="bg-nav shadow-md py-2 px-1 mb-2 rounded-md hover:cursor-pointer"
                    onClick={() =>
                      navigate(
                        `${
                          time.complete
                  ...
                  >
                    ...
                    <p className="font-semibold text-xs ml-3">
                      Applicants: {x.applicants.length}
                    </p>
                    <p className="font-semibold text-xs  ml-3">
