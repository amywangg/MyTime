...
function PendingPosting({ closed }) {
  const [posting, setPosting] = useState();
  const [postDate, setPostDate] = useState();
  const [timeslots, setTimeslots] = useState();
  const [message, setMessage] = useState(null);
  const { completePostings, postingLoading, updateStatus } =
  ...
  useEffect(() => {
    if (!postingLoading) {
      ...
      setPostDate(
        new Date(
          completePostings.filter((post) => post.id === parseInt(id))[0]?.date
        )
      );
      setPosting(
  ...
  }, [postingLoading, completePostings]);
                            ...
                            {applicant.status === "selected" ? (
                              <div className="flex justify-center self-center items-center">
                                <button
                                  ...
                                  onClick={() => {
                                    setMessage("Successfully signed!");
                                    setTimeout(() => {
                                      setMessage(null);
                                    }, [2000]);
